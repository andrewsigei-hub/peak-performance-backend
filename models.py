...
class User(Base):
    ...
    google_id = Column(String, unique=True, nullable=True) ## Can be empty when user hasnt logged in
...
class Workout(Base):
    ...
    exercises = relationship("Exercise", back_populates="workout", cascade="all, delete-orphan") ## If a workout is deleted all exercise are too
...
class Exercise(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    workout_id = Column(Integer, ForeignKey('workouts.id'), nullable=False) # Foreign Key
    name = Column(String, nullable=False)
